...
toSSMCommands (CFCall id 0) = [S (Sbsr id)]
toSSMCommands (CFCall id i) = [S (Sbsr id), S (Sajs (~i))]
...
toSSMCommands (BranchMatch [(i, name):cases] id) = [S (Slds 0): S (Sldc i): S Seq: S (Sbrt name): toSSMCommands (BranchMatch cases id)]
toSSMCommands (BranchMatch [] id) = [SL id (Sajs -1)]
...
toSSMCommandsExp (EFCall name 0) = [S (Sbsr name), S SldrRR]
toSSMCommandsExp (EFCall name i) = [S (Sbsr name), S (Sajs (~i)), S SldrRR]
...
fst`			= [SL "fst" (Slds -1), S (Sldmh 0 2), S (Sajs -1), S SstrRR, S Sret]
